...
AAD_CLIENT_ID=$(jq -r .name ${SHARED_DIR}/osServicePrincipal.json)
AZURE_REGION=ppe2
SSH_KEY=$(yq r install-config.yaml sshKey | xargs)
BASE_DOMAIN=$(yq r install-config.yaml baseDomain)
TENANT_ID=$(jq -r .tenant ${SHARED_DIR}/osServicePrincipal.json)
AAD_CLIENT_SECRET=$(jq -r .password ${SHARED_DIR}/osServicePrincipal.json)
APP_ID=$(jq -r .appId ${SHARED_DIR}/osServicePrincipal.json)
...
az cloud register \
    -n PPE \
    --endpoint-resource-manager "https://management.ppe2.stackpoc.com" \
    --suffix-storage-endpoint "ppe2.stackpoc.com" 
az cloud set -n PPE
