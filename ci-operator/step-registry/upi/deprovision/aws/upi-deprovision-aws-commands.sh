...
while IFS= read -r i; do
    ...
    queue ${SHARED_DIR}/nodes/$i/console aws ec2 get-console-output --instance-id "${i}" --output text
...
for STACK_SUFFIX in compute-2 compute-1 compute-0 control-plane bootstrap proxy security infra vpc
do
    aws cloudformation wait describe-stacks --stack-name "${CLUSTER_NAME}-${STACK_SUFFIX}"
done